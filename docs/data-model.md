...
- Legacy archives without verified account binding require one explicit `--adopt-source` once the source exposes a verified account identity. Adoption rejects accountless sources, even for an empty archive; a matching store fingerprint is not account proof. Until account metadata returns, read retained history with `--sync never`. Use a separate `--db` for another account or `--restore` for intentional source replacement.
...
Native duplicate rows contribute all raw observations before collapse. Missing LIDs are absent evidence, and sparse duplicates preserve nonempty contact fields from the same snapshot. Conflicting normalized LIDs retain existing contact display fields; a new conflicted contact has only JID and evidence, with unknown display and scalar LID. Equivalent bare/full observations do not conflict. Canonical snapshot contact keys must be unique, and malformed or duplicate evidence keys reject before writes. Exact restore preflight ignores destination contact evidence, then replaces it with incoming evidence; legacy snapshots seed only their own scalar values with unknown provenance.
